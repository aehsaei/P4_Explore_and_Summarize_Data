...
# Change 'C.E.O.' -> 'CEO' to remove duplicates under a different name
cc$contbr_occupation <- gsub("C.E.O.", "CEO", cc$contbr_occupation)
...
ggplot(data = cand_gender_df, aes(x = cand_gender)) +
  geom_dotplot(aes(fill = cand_gender), binwidth = 0.1) +
  xlab("Candidate Gender") +
  guides(fill=guide_legend(title="Candidate Gender"))
```

We can visually see there are many more male candidates than female candidates. Let's gather gender data on the contributors now.
...
political parties. There were only singular candidates from the green, independent, and libertarian parties who received contributions from North Carolinians in 2016.
...
ggplot() +
  ...
  xlab("Contribution Amounts (log10) in USD") +
  ggtitle("Contribution Amounts by Party") +
  guides(fill=guide_legend(title="Candidate Party"))
```
...
# State outline
state_plot <- geom_polygon(data = nc, 
                           aes(x = long, y = lat, group = group), 
                           colour = "black", 
                           fill = "white")

# City point locations
city_points_plot <- geom_point(data = nc_cities, 
                                 aes(x = long, y = lat), 
                                 color = "white", 
                                 shape = 18)

# City names
city_names_plot <- geom_text(data = subset(nc_cities, 
                                           name == "Asheville"  | 
                                           name == "Charlotte"  | 
                                           name == "Raleigh"    | 
                                           name == "Greensboro" |
...
# Democrat locations
dem_locations_plot <- geom_point(data = subset(cc, cand_party == "democrat"), 
                                 aes(x = longitude, 
                                     y = latitude, 
                                     fill = cand_party), 
                                 alpha = 1/110, 
...
rep_locations_plot <- geom_point(data = subset(cc, cand_party == "republican"), 
                                 aes(x = longitude, 
                                     y = latitude, 
                                     fill = cand_party), 
                                 alpha = 1/110, 
...
# Override legend title and alpha
legend_party_or <- guides(fill = guide_legend(override.aes = list(alpha = 1), 
                                              title="Party"))

# Remove whitespace around plots
theme_no_whitespace <-  theme(axis.text.x = element_blank(),
...
# Remove legend and whitespace
theme_no_legend_and_whitespace <-  theme(legend.position = "none", 
...
# Plot democrat locations on a map
p1 <- ggplot() +
  state_plot +
  dem_locations_plot +
  city_points_plot +
...
p3 <- ggplot() +
  ...
  city_points_plot +
  coord_fixed() +
  ggtitle("Overlay Party Contributor Locations") +
  theme_no_legend_and_whitespace

grid.arrange(p1, p2, p3, ncol = 1, top = "Contributor Locations by Party")
```

The independent democrat and republican plots don't show much of a difference 
and it appears that both parties have strong support in the same areas. We can 
see this more clearly in the third plot where I overlay the locations. The third
plot shows the there are slightly more democrat contributions around Asheville, 
NC as the main discernible difference.
...
ggplot(data = filter(cc, 
  ...
  ggtitle("Top Contributer Occupations") +
  xlab("Contributor Occupations") +
  ylab("Number of Contributions") +
  theme(text = element_text(size=18),
        axis.text.x = element_text(angle=45, hjust=1))
```
...
```{r echo=TRUE, contributions_by_occupations_plot}
...
grid.arrange(p1, p2, ncol = 2, top = "Top Contributor Occupations")
```
...
```{r echo=TRUE, contributions_by_occupations_plot_no_retired}
p1 <- ggplot(data = subset(cc, 
                           contbr_occupation %in% 
...
grid.arrange(p1, p2, ncol = 2, top = "Top Contributor Occupations (Limited)")
```
...
top the chart on the republican plot. Let's see which occupations made the 
largest contribution amounts and compare this plot to the contributions plot we 
just generated.

```{r echo=TRUE, contribution_amounts_by_occupations_plot}
# Create a data frame for the democrat contributors in descending order of amount
democrat_high_contbr <- subset(cc, cand_party == "democrat") %>%
  group_by(contbr_occupation) %>%
  summarise(sum_contb = sum(contb_receipt_amt),
            n = n()) %>%
  arrange(desc(sum_contb), desc(n))


# Top Democrat Contributing Occupations
p1 <- ggplot(data = head(democrat_high_contbr, 10),
       aes(x = contbr_occupation, y = sum_contb)) +
  geom_bar(fill = I('#3498DB'), color = I('black'), stat = "identity") +
  scale_y_continuous(breaks = seq(0, 
                                  max(democrat_high_contbr$sum_contb), 
                                  max(democrat_high_contbr$sum_contb)/10),
                     labels = comma) +
...
# Create a data frame for the republican contributors in descending order of amount
republican_high_contbr <- subset(cc, cand_party == "republican") %>%
  group_by(contbr_occupation) %>%
  summarise(sum_contb = sum(contb_receipt_amt),
            n = n()) %>%
  arrange(desc(sum_contb), desc(n))
...
# Top republican Contributing Occupations
p2 <- ggplot(data = head(republican_high_contbr, 10),
       aes(x = contbr_occupation, y = sum_contb)) +
  geom_bar(fill = I('#E74C3C'), color = I('black'), stat = "identity") +
  scale_y_continuous(breaks = seq(0, 
                                  max(republican_high_contbr$sum_contb), 
                                  max(republican_high_contbr$sum_contb)/10),
                     labels = comma) +
  ylab("Total Contribution Amount in USD") +
  xlab("Occupations") +
  ggtitle("Republican") +
...
grid.arrange(p1, p2, ncol = 2, top = "Total Contribution Amounts by Occupation")
```

We can see the highest total money came from retired North Carolinians 
in 2016 for both parties. This is surprising considering how much larger the 
total amount is relative to the other top contributing occupations. It's also 
surprising that the second largest amount of money for the democrats was donated 
by unemployed contributors. 

### Which gender made more contributions?
Let's next look at gender and how it played a role in the contributions to 
candidates. First let's see a breakdown of contributions by the contributor
genders.
...
p2 <- ggplot(data = subset(cc, cc$contb_receipt_amt <= 2700), 
  ...
  xlab("Contributor Gender") +
  ylab("Contribution Amounts in USD (log10)") +
  ggtitle("Contribution Amounts") +
  theme(text = element_text(size=12)) +
  guides(fill=guide_legend(title="Contributor Gender"))
...
contributed $119 on average in 2016. 

### Which gender received more contributions from either gender?
Let's next look to see if there were a disproportionate amount of contributions 
to each candidate gender.

```{r echo=FALSE, gender_contributions_plot}
ggplot() +
...
democratic front-running candidate Hillary Clinton. 

### Which candidate received more contributions from either gender?
Let's take a look at a plot of all the candidates' coontributions broken down
by gender to see if there are any unusual relationships.
...
ggplot() +
  ...
  xlab("Candidate") +
  ylab("Number of Contributions") +
  ggtitle("Candidate Contributions by Contributor Gender") +
  theme(text = element_text(size=12),
        axis.text.x = element_text(angle=45, hjust=1)) +
  guides(fill=guide_legend(title="Contributor Gender"))
```

This shows that Hillary Clinton received the largest amount of female 
contributions among the candidates in 2016. Of the top six candidates here, she 
is the only candidate to have a majority of female contributors. Let's look at 
contribution amounts broken down by gender next to see if the most number of 
contributions also resulted in the most total money contributed.
...
ggplot() +
  ...
  xlab("Candidate") +
  ylab("Contribution Amounts in USD") +
  ggtitle("Candidate Contribution Amounts by Gender") +
  theme(text = element_text(size=12),
        axis.text.x = element_text(angle=45, hjust=1)) +
  guides(fill=guide_legend(title="Contributor Gender"))
```
...
contributors gave slightly less to nearly every candidate listed. Ben Carson was 
the exception with identical 1st and 3rd quartiles and median values for female 
and male contribution amounts.

### Which occupations contributed to the Clinton campaign the most?
Let's look at the Hillary Clinton supporters' occupations broken down by gender 
next.
...
ggplot() +
  ...
  ylab("Number of Contributions") +
  ggtitle("Hillary Contributions by Occupations and Gender") +
  theme(text = element_text(size=12),
        axis.text.x = element_text(angle=45, hjust=1)) +
  guides(fill=guide_legend(title="Contributor Gender"))
```

Hillary Clinton had a very large contributor base that was female and retired. 

## Final Plots and Summary
#### Plot 1: Highest Contributing Occupations
```{r echo=FALSE, target_the_high_donors_plot}
# Top Democrat Contributing Occupations
p1 <- ggplot(data = head(democrat_high_contbr, 10),
       aes(x = contbr_occupation, y = sum_contb)) +
  geom_bar(fill = I('#3498DB'), color = I('black'), stat = "identity") +
...
grid.arrange(p1, p2, ncol = 2, top = "Total Contribution Amounts by Occupation")
```
In this plot we can see the breakdown by top contributing occupations for each party. There is some very valuable information in this plot in relation to which contributor occupations donated the most money. It was very surprising to see retired contributors as the largest donors relative to the other occupations. Using this plot, parties could target their future fundraising efforts to contributors that are more likely to donate.

#### Plot 2: Women Supported the first woman nominee
```{r echo=FALSE, women_support_plot}
ggplot() +
  geom_bar(data = subset(cc, table(cand_last_nm)[cand_last_nm] > 1000), 
           aes(x = cand_last_nm, fill = contbr_gender), 
           position = "dodge",
...
namely the Democrats. By looking at the number of contributions from both genders to each of the candidates, we can see that Hillary was supported by women in a big way. Clinton received a very large number of contributions from women relative to the other candidates and I suspect the historical nature of her nomination could have been a factor.

#### Plot 3: North Carolina is a "Purple state"
```{r echo=FALSE, purple_state_plot}
# Plot democrat locations on a map
p1 <- ggplot() +
  state_plot +
  dem_locations_plot +
...
A purple state refers to a swing state where both Democratic and Republican candidates receive strong support without an overwhelming majority of support for either party. Purple states are also often referred to as battleground states. In this third plot with the overlay of each party's color we can see clearly that North Carolina would fit into this category. This was some surprising information in these plots. I was surprised to see the contribution locations for both parties in the same areas as well as the amount of contributions from rural areas.
