...
cc <- read.csv('/Users/aehsaei/Documents/DataAnalyst/Project_4/P00000001-NC.csv', row.names=NULL)
```
...
cc <- cc[c("cmte_id", "cand_id", "cand_nm", "cand_first_nm", "cand_last_nm", "cand_gender", "cand_party", "contbr_nm", "contbr_first_nm", "contbr_last_nm", "contbr_gender", "contbr_occupation", "contbr_employer", "contb_receipt_amt", "contb_receipt_dt", "contbr_city", "contbr_st", "contbr_zip", "latitude", "longitude", "receipt_desc", "election_tp", "memo_cd", "memo_text", "form_tp", "file_num", "tran_id")]
...
ggplot(data = subset(cc, table(cand_last_nm)[cand_last_nm] > 1000), aes(x = cand_last_nm, fill = cand_party)) +
  geom_bar(color = I("black")) +
  scale_fill_manual(values=c(I("#3498DB"),I('#E74C3C'), "grey", "black","grey")) +
  xlab("Candidates with > 1000 Contributions") +
  ylab("Number of Contributions") +
...
ggplot(data = subset(cc, contb_receipt_amt <= 2700), aes(x = contb_receipt_amt)) +
  geom_histogram(binwidth = 10, fill = I('#27AE60'), color = I('#27AE60')) +
  scale_x_continuous(breaks = seq(0, 3000, by = 500), "Contributions in dollars")
```
...
ggplot() +
  geom_histogram( data = subset(cc, contb_receipt_amt <= 2700), aes(x = contb_receipt_amt, fill = cand_party), binwidth = .6, position = "dodge") +
  scale_fill_manual(values=c(I("#3498DB"), I("#27AE60"), "grey", "black", I('#E74C3C'))) +
  scale_x_log10() +
  xlab("Contributions in dollars (log10)")
```
...
ggplot(data = filter(cc, cc$contbr_occupation %in% head(top_contributing_occupations$contbr_occupation, 10)),
...
p1 <- ggplot(data = subset(cc, contbr_occupation %in% head(democrat_occupations$contbr_occupation, 10) & cand_party == "democrat"),
       aes(x = contbr_occupation)) +
...
# Create a data frame for the republican contributors in descending order of count
republican_occupations <- subset(cc, cand_party == "republican") %>%
  group_by(contbr_occupation) %>%
  summarise(n = n()) %>%
  arrange(desc(n))

p2 <- ggplot(data = subset(cc, contbr_occupation %in% head(top_contributing_occupations$contbr_occupation, 10) & cand_party == "republican"),
       aes(x = contbr_occupation)) +
...
p1 <- ggplot(data = subset(cc, contbr_occupation %in% head(democrat_occupations$contbr_occupation, 12) & 
                               cand_party == "democrat" &
...
p2 <- ggplot(data = subset(cc, contbr_occupation %in% head(top_contributing_occupations$contbr_occupation, 12) & 
...
p1 <- ggplot(data = cc, aes(x = contbr_gender)) +
  geom_bar(aes(fill = contbr_gender)) +
  xlab("Contributor Gender") +
  ylab("Contributions") +
  ggtitle("Contributions by Gender") +
  theme(text = element_text(size=12))

p2 <- ggplot(data = subset(cc, cc$contb_receipt_amt <= 2700), aes(x = contbr_gender, y = contb_receipt_amt)) +
  scale_y_log10() +
  geom_boxplot(aes(fill = contbr_gender)) +
  xlab("Contributor Gender") +
  ylab("Contribution Amounts") +
  ggtitle("Contribution Amounts by Gender") +
  theme(text = element_text(size=12))
...
ggplot() +
  geom_bar(data = subset(cc, contbr_gender == "female"), aes(x = contbr_gender, fill = cand_gender), width = 0.25, position="dodge") +
  geom_bar(data = subset(cc, contbr_gender == "male"), aes(x = contbr_gender, fill = cand_gender), width = 0.25, position="dodge") +
  xlab("Contributor Gender") +
  ylab("Contributions") +
...
ggplot() +
  geom_bar(data = subset(cc, table(cand_last_nm)[cand_last_nm] > 1000), aes(x = cand_last_nm, fill = contbr_gender), position = "dodge") +
  xlab("Candidate") +
...
clinton_male_contributor_occupations <- subset(cc, cc$contbr_gender == "male") %>%
  group_by(contbr_occupation) %>%
  summarise(n = n()) %>%
  arrange(desc(n)) %>%
  head(10)
...
clinton_female_contributor_occupations <- subset(cc, cc$contbr_gender == "female") %>%
  group_by(contbr_occupation) %>%
  summarise(n = n()) %>%
  arrange(desc(n)) %>%
  head(10)
...
ggplot() +
  geom_bar(data = subset(cc, cc$cand_last_nm == "Clinton" & 
                             (cc$contbr_occupation %in% clinton_female_contributor_occupations$contbr_occupation |
                              cc$contbr_occupation %in% clinton_male_contributor_occupations$contbr_occupation)),
           aes(x = contbr_occupation, fill = contbr_gender), position = "dodge") +
...
ggplot() +
  geom_boxplot(data = subset(cc, cc$contb_receipt_amt <= 2700 & 
               ...
               aes(x = cand_last_nm, y = contb_receipt_amt, fill = contbr_gender), position = "dodge") +
  scale_y_log10() +
  xlab("Candidate") +
  ylab("Contribution Amounts") +
